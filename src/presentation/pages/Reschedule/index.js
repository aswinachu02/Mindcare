...
const RescheduleSession = () => {
  ...
  const [{ patientname, username }] = useAuthStore();
  const [
    {
    ...
    { handleGetSession, handleBookSession },
  ] = useSessionsStore();
  ...
  const handleBookNow = () => {
    if (!selectedDate) return message.error("Please select a date!");
      ...
      const sid = selectedSession?.sid;
      const d_uname = selectedDoctor?.username;
      return handleBookSession(
        ...
        () => {
          notification.success({
            ...
            description: `New session booking success for ${patientname} with ${selectedDoctor?.name}`,
          });
          navigate("/plan");
        }
      );
  ...
  return (
    <MobileLayout>
      <Header>Reschedule Session</Header>
      <LoadAndResult
